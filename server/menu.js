module.exports = [
		{
			id : 1,
			name : 'Margherita',
			ingredients : [ 'Sos', 'Ser' ],
			price : 14.90
		},
		{
			id : 2,
			name : 'Funghi ',
			ingredients : [ 'Sos', 'Ser', 'Pieczarki' ],
			price : 16.90
		},
		{
			id : 3,
			name : 'Vesuvio',
			ingredients : [ 'Sos', 'Ser', 'Szynka' ],
			price : 17.90
		},
		{
			id : 4,
			name : 'Salami',
			ingredients : [ 'Sos', 'Ser', 'Salami' ],
			price : 17.90
		},
		{
			id : 5,
			name : 'Capriciosa',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Pieczarki' ],
			price : 19.90
		},
		{
			id : 6,
			name : 'Hawajska',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Ananas' ],
			price : 19.90
		},
		{
			id : 7,
			name : 'Toscana',
			ingredients : [ 'Sos', 'Ser', 'Salami', 'Pieczarki' ],
			price : 19.90
		},
		{
			id : 8,
			name : 'Tono',
			ingredients : [ 'Sos', 'Ser', 'Tuńczyk', 'Cebula' ],
			price : 20.90
		},
		{
			id : 9,
			name : 'Peperone',
			ingredients : [ 'Sos', 'Ser', 'Salami', 'Papryka peperoni',
					'Cebula' ],
			price : 21.90
		},
		{
			id : 10,
			name : 'Sandra',
			ingredients : [ 'Sos', 'Ser', 'Salami', 'Pomidor', 'Cebula' ],
			price : 21.90
		},
		{
			id : 11,
			name : 'Pescatore',
			ingredients : [ 'Sos', 'Ser', 'Tuńczyk', 'Cebula', 'Kapary' ],
			price : 22.90
		},
		{
			id : 12,
			name : 'Bolonia',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Pieczarki', 'Papryka',
					'Cebula' ],
			price : 23.90
		},
		{
			id : 13,
			name : 'Milano',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Kabanos', 'Papryka',
					'Kukurydza' ],
			price : 24.90
		},
		{
			id : 14,
			name : 'Vegetariana ',
			ingredients : [ 'Sos', 'Ser', 'Pieczarki', 'Papryka', 'Kukurydza',
					'Cebula' ],
			price : 22.90
		},
		{
			id : 15,
			name : 'Rzeźnicka',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Salami', 'Bekon',
					'Kabanos', 'Cebula' ],
			price : 26.90
		},
		{
			id : 16,
			name : 'Pizza Kebab',
			ingredients : [ 'Sos', 'Ser', 'Kebab drobiowy', 'Cebula',
					'Kapusta pekińska', 'Pomidor' ],
			price : 26.90
		},
		{
			id : 17,
			name : 'La Mafia',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Bekon', 'Pieczarki',
					'Cebula' ],
			price : 24.90
		},
		{
			id : 18,
			name : 'Meksykańska',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Salami', 'Papryka',
					'Cebula', 'Kukurydza' ],
			price : 25.90
		},
		{
			id : 19,
			name : 'MARIO',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Salami', 'Pomidor',
					'Camembert' ],
			price : 25.90
		},
		{
			id : 20,
			name : 'Rimini',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Salami', 'Pieczarki',
					'Cebula' ],
			price : 24.90
		},
		{
			id : 21,
			name : 'Farmerska',
			ingredients : [ 'Sos', 'Ser', 'Bekon', 'Boczek', 'Kabanos',
					'Pieczarki', 'Cebula' ],
			price : 25.90
		},
		{
			id : 22,
			name : 'Kebab Pizza 2',
			ingredients : [ 'Sos', 'Ser', 'Kebab drobiowy', 'Cebula',
					'Papryka peperoni' ],
			price : 24.90
		},
		{
			id : 23,
			name : 'Americana (grube ciasto)',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Kebab drobiowy',
					'Pieczarki', 'Cebula' ],
			price : 25.90
		}, {
			id : 24,
			name : 'Frutti di Mare',
			ingredients : [ 'Sos', 'Ser', 'Tuńczyk', 'Krewetki', 'Małże' ],
			price : 25.90
		}, {
			id : 25,
			name : 'Uczta Serowa ',
			ingredients : [ 'Sos', 'Ser', 'Lazur', 'Camembert', 'Extra ser' ],
			price : 25.90
		}, {
			id : 26,
			name : 'Brocoli',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Brokuły', 'Camembert' ],
			price : 23.90
		}, {
			id : 27,
			name : 'Sicilia',
			ingredients : [ 'Sos', 'Ser', 'Szynka', 'Ananas', 'Kukurydza' ],
			price : 21.90
		} ];
